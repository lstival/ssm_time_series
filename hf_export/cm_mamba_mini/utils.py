...
def time_series_2_recurrence_plot(x):
    ...
    PyTorch-native version to avoid GPU-CPU-GPU transfers.
    Returns:
        torch.Tensor: (B, L, L) or (B, C, L, L)
    """
    if not isinstance(x, torch.Tensor):
        x = torch.from_numpy(np.asarray(x)).float()
    
    # x shape could be (B, L) or (B, C, L)
    if x.ndim == 1:
        x = x.unsqueeze(0)
    ...
    # If it's (B, C, L), we want (B, C, L, L)
    # If it's (B, L, 1), we want (B, L, L)

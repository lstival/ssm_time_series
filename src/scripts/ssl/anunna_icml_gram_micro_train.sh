#!/bin/bash
#SBATCH --comment=icml_gram_micro_train
#SBATCH --time=720
#SBATCH --mem=32000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_gram_micro/train_%j.out
#SBATCH --error=logs/icml_gram_micro/train_%j.err
#SBATCH --job-name=icml_gram_m
...
echo "Training GRAM MICRO on ICML datasets (supervised-domain baseline)"
echo "  model_dim=64, depth=6, emb=32, ctx=512, epochs=50"
echo "  datasets: ett_h1/h2, ett_m1/m2, electricity_hourly, traffic_hourly, exchange_rate"

time python3 "${SRC}/gram_training.py" \

#!/bin/bash
#SBATCH --comment=simclr_bi_nano_probe_psn
#SBATCH --time=60
#SBATCH --mem=16000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/simclr_bimodal_nano/probe_psn_%j.out
#SBATCH --error=logs/simclr_bimodal_nano/probe_psn_%j.err
#SBATCH --job-name=simclr_bi_n_psn
...
echo "Probing [per-series-norm] simclr_bi nano: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/probe_simclr_bimodal_nano_psn \

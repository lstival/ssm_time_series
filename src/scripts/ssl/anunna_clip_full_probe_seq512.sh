#!/bin/bash
#SBATCH --comment=clip_full_probe_seq512
#SBATCH --time=720
#SBATCH --mem=64000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/clip_full/probe_seq512_%j.out
#SBATCH --error=logs/clip_full/probe_seq512_%j.err
#SBATCH --job-name=clip_f_seq512
...
echo "Zero-shot probe — CLIP full (seq_len=512, frozen encoder): ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/clip_full_seq512 \

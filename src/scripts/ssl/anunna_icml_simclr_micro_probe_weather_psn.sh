#!/bin/bash
#SBATCH --comment=icml_simclr_micro_probe_weather_psn
#SBATCH --time=60
#SBATCH --mem=16000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_simclr_micro/probe_weather_psn_%j.out
#SBATCH --error=logs/icml_simclr_micro/probe_weather_psn_%j.err
#SBATCH --job-name=simclr_m_wx
...
echo "Probing [weather per-series-norm] simclr micro: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/icml_simclr_micro_weather_psn \

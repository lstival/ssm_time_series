#!/bin/bash
#SBATCH --comment=byol_bi_nano_probe_solar
#SBATCH --time=180
#SBATCH --mem=64000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/byol_bimodal_nano/probe_solar_%j.out
#SBATCH --error=logs/byol_bimodal_nano/probe_solar_%j.err
#SBATCH --job-name=byol_bi_n_sol
...
echo "Probing [solar] byol_bi nano: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --batch_size 1 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/byol_bi_nano_solar \

#!/bin/bash
#SBATCH --comment=icml_clip_micro_probe_5pct
#SBATCH --time=360
#SBATCH --mem=32000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_clip_micro/probe_5pct_%j.out
#SBATCH --error=logs/icml_clip_micro/probe_5pct_%j.err
#SBATCH --job-name=icml_clip_5pct
...
echo "Few-shot 5% probe — CLIP micro ICML checkpoint: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/icml_clip_micro_5pct \

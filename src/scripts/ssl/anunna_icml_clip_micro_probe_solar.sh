#!/bin/bash
#SBATCH --comment=icml_clip_micro_probe_solar
#SBATCH --time=180
#SBATCH --mem=64000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_clip_micro/probe_solar_%j.out
#SBATCH --error=logs/icml_clip_micro/probe_solar_%j.err
#SBATCH --job-name=clip_m_sol
...
echo "Probing [solar] clip micro: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --batch_size 1 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/icml_clip_micro_solar \

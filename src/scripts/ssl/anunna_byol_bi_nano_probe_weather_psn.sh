#!/bin/bash
#SBATCH --comment=byol_bi_nano_probe_weather_psn
#SBATCH --time=60
#SBATCH --mem=16000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/byol_bimodal_nano/probe_weather_psn_%j.out
#SBATCH --error=logs/byol_bimodal_nano/probe_weather_psn_%j.err
#SBATCH --job-name=byol_bi_n_wx
...
echo "Probing [weather per-series-norm] byol_bi nano: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/probe_byol_bimodal_nano_weather_psn \

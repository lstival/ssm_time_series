#!/bin/bash
#SBATCH --comment=icml_gram_micro_probe_weather_psn
#SBATCH --time=60
#SBATCH --mem=16000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_gram_micro/probe_weather_psn_%j.out
#SBATCH --error=logs/icml_gram_micro/probe_weather_psn_%j.err
#SBATCH --job-name=gram_m_wx
...
echo "Probing [weather per-series-norm] gram micro: ${CHECKPOINT_DIR}"
...
time python3 "${SRC}/experiments/probe_lotsa_checkpoint.py" \
    ...
    --probe_epochs 20 \
    --results_dir /home/WUR/stiva001/WUR/ssm_time_series/results/icml_gram_micro_weather_psn \

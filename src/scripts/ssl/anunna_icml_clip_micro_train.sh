#!/bin/bash
#SBATCH --comment=icml_clip_micro_train
#SBATCH --time=720
#SBATCH --mem=32000
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/icml_clip_micro/train_%j.out
#SBATCH --error=logs/icml_clip_micro/train_%j.err
#SBATCH --job-name=icml_clip_m
...
echo "Training CLIP bimodal MICRO on ICML datasets (supervised-domain baseline)"
echo "  model_dim=64, depth=6, emb=32, ctx=512, epochs=50, alignment=clip_symm"
